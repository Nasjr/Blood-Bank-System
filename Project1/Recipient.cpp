...
bool Recipient::ischar(char c)
{
    return (c >= 'a' && c <= 'z')||
        (c >= 'A' && c <= 'Z');
}
bool Recipient::isDigit(char c)
{
   return (c >= '0' && c <= '9');
}

bool Recipient::is_valid(string email)
{
    if (!ischar(email[0])) {
...
bool Recipient::empty(String^ x)
{
    return x->IsNullOrEmpty(x);
}

bool Recipient:: check_id(string x)
{
    bool notint = 0;
    if (x.size() == 14) {
        for (int i = 0; i < 14; i++) {
...
bool Recipient::check_uname(string x)
{
    int size = x.size();
...
string Recipient::fStos(String^ x)
{
    msclr::interop::marshal_context context;
...
void Recipient::takeData()
{
    string email = "R" + fStos(textBox2->Text) ;
    char c;
    if (radioButton5->Checked)
        c = 'm';
    else
        c= 'f';
     recip->push_back(recipient(fStos(textBox5->Text), fStos(textBox6->Text), fStos(textBox3->Text),
        fStos(textBox1->Text),email, fStos(comboBox1->Text), fStos(comboBox2->Text), fStos(comboBox3->Text)
        , fStos(textBox4->Text),c, fStos(comboBox4->Text)));
